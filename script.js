...
function runSceneEnterHook(name) {
  switch (name) {
    case "couple":
      animateCoupleFrame("coupleDuo1", 900);
      break;
    case "memories":
      initMemories();
      break;
...
function animateCoupleFrame(duoId, delay) {
  const duo = document.getElementById(duoId);
  if (!duo) return;
  const frame = duo.closest(".couple-frame");
...
function setupCoupleInteractions() {
  ...
  // Tap/click either character for a little "bump into each other" reaction:
  // both halves nudge in opposite directions, plus a small sparkle burst.
  document.querySelectorAll("[data-couple-duo]").forEach((duo) => {
    duo.addEventListener("click", () => {
      const halves = duo.querySelectorAll(".couple-img");
      halves.forEach((half) => {
        half.classList.remove("is-nudged");
        void half.offsetWidth; // restart animation on repeated taps
        half.classList.add("is-nudged");
      });
      spawnSparkleBurst(duo);
    });

    // Graceful per-character fallback if a half fails to load.
    duo.querySelectorAll("[data-couple-half]").forEach((img) => {
      img.addEventListener(
        "error",
        () => {
          const which = img.dataset.coupleHalf;
          const frame = duo.closest(".couple-frame");
          img.remove();
          ...
          fallback.textContent =
            which === "boy"
              ? "Add assets/couple-boy.png"
              : "Add assets/couple-girl.png";
          duo.appendChild(fallback);
...
function spawnSparkleBurst(anchorEl) {
  ...
  const frame = anchorEl.closest(".couple-frame") || anchorEl;
  if (!frame) return;
  const symbols = ["✨", "💙", "⭐"];
  const count = 6;
